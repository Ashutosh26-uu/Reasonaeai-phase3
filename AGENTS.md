...
- [`SPEC.md`](./.context/SPEC.md) — current product specification, architecture, security invariants, and approved technology stack.
- [`PHASE.md`](./.context/PHASE.md) — current delivery phase, active scope, status, exit criteria, and immediate next work.
- [`FUTURE.md`](./.context/FUTURE.md) — accepted ideas intentionally deferred beyond the current phase.
- [`PLAN.md`](./.context/PLAN.md) — original Phase 3 plan and delivery rationale. It is historical planning context; `.context/SPEC.md` and `.context/PHASE.md` describe the current contract and execution state.

When these files conflict, use this precedence:

1. Security and data-integrity invariants in `.context/SPEC.md`.
2. Current scope and exit criteria in `.context/PHASE.md`.
3. Current product contract in `.context/SPEC.md`.
4. Deferred intent in `.context/FUTURE.md`.
5. Historical intent in `.context/PLAN.md`.
...
- Update `.context/SPEC.md` when an accepted product requirement, system boundary, public contract, technology choice, or security invariant changes.
- Update `.context/PHASE.md` when work starts, completes, becomes blocked, changes order, or satisfies/fails an exit criterion.
- Add an item to `.context/FUTURE.md` when a worthwhile feature is explicitly deferred. Remove it when it moves into `.context/PHASE.md` or is rejected, recording the decision.
- Keep `.context/PLAN.md` unchanged unless the historical plan itself is being corrected. Do not use it as a live task tracker.
...
1. Read `.context/SPEC.md` and `.context/PHASE.md`; read `.context/FUTURE.md` when considering scope expansion.
...
5. Implement the real behavior. A change is either complete or explicitly tracked as in progress in `.context/PHASE.md`.
...
An implementation may be concluded **complete** only when every applicable criterion below has objective evidence. If a criterion does not apply, record why in the pull request or task evidence. Any unmet applicable criterion means the work remains in progress or blocked in `.context/PHASE.md`.
...
13. **Context is current** — `.context/SPEC.md`, `.context/PHASE.md`, `.context/FUTURE.md`, `AGENTS.md`, API documentation, and operational instructions reflect the implemented reality.
...
- Mark the work blocked or in progress in `.context/PHASE.md`.
...
- Use Node.js for repository JavaScript execution. Do not introduce Bun or another runtime without a measured, repository-wide decision recorded in `.context/SPEC.md`.
