...
- Added `@reasonateai/cto-runtime` with the branded full-capability ReasonateAI CTO, scout/coder/debugger workers and ephemeral orchestrator-authored custom agents sharing one definition interface, hidden title and compaction agents, shared verified run scope, and Mastra `AgentController` composition.
...
- Implement the `read` tool as the single front door over filesystem paths, archives, documents, notebooks, SQLite, images, and the registered resource schemes, including structural summaries for parseable code, elided-range recovery, artifact spilling, and content anchors for editable resources.
...
- Add the tool registry that makes the run's tool inventory and each agent's tool eligibility derived from real tool metadata rather than hand-listed names.
