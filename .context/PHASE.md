...
- Hardened `@reasonateai/sandbox` and integrated `@reasonateai/sandbox/mastra`:
  - Created `MastraWorkspaceSandboxAdapter` and `createMastraSandbox` bridging ReasonateAI's `DockerSandbox` with `@mastra/core/workspace`'s `WorkspaceSandbox` contract.
  - Replaced `@mastra/docker` in `apps/api/src/mastra/workspace.ts` with `@reasonateai/sandbox/mastra`, removing named volume leaks and trimming 22 unneeded transitive dependencies.
