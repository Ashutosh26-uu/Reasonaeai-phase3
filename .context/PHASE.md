...
- Complete the `read` tool's sandbox-backed format and resource handlers. The current custom Mastra tools bind `read`, hash-bound `write`, and hashline `edit` to the verified per-run workspace filesystem; `read` handles project paths and directories; registered resource URIs need a run-specific authorized handler that is not wired in this launch slice, while archives, documents, notebooks, SQLite, images, structural summaries, and artifact spilling still need sandbox-compatible adapters.
...
- Reintroduce ephemeral custom-agent delegation through `AgentController`; the previous standalone custom-agent tool was removed because it bypassed controller governance and could receive arbitrary injected tools.
