...
// packages/code/src/tools/hashline/fs.ts.
...
export function isNotFound(error: unknown): boolean {
  ...
  // `node:fs` errors carry `code`; `Error` does not declare it.
  if (
...
export abstract class Filesystem {
  ...
  delete(path: string): Promise<void> {
    return Promise.reject(
      new Error(`Filesystem does not support delete: ${path}`)
    );
  }
   ...
   */
  move(from: string, to: string, _content?: string): Promise<void> {
    return Promise.reject(
      new Error(`Filesystem does not support move: ${from} -> ${to}`)
    );
  }
...
export class InMemoryFilesystem extends Filesystem {
  ...
  override move(from: string, to: string, content?: string): Promise<void> {
    ...
    if (existing === undefined) {
      return Promise.reject(new NotFoundError(from));
    }
...
/**
 * Disk-backed {@link Filesystem} using `node:fs/promises`. The default for CLI
...
export class NodeFilesystem extends Filesystem {
  ...
  override async move(
    ...
    if (content !== undefined) {
      await this.writeText(to, content);
    ...
    try {
      ...
      if (isNotFound(error)) {
        throw new NotFoundError(from, { cause: error });
      }
      throw error;
    }
  }
