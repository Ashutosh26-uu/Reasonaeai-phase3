...
// packages/code/src/tools/hashline/format.ts.
...
/** Hunk-header keyword: `SWAP.BLK N:` resolves N to a block range and replaces its span. */
export const HL_REPLACE_BLOCK_KEYWORD = "SWAP.BLK";
/** Hunk-header keyword: `DEL.BLK N` resolves N to a block range and deletes its span. */
export const HL_DELETE_BLOCK_KEYWORD = "DEL.BLK";
/** Hunk-header keyword: `INS.BLK.POST N:` inserts after the last line of the block at N. */
...
const TRAILING_INLINE_SPACE_RE = /[ \t\r]+(?=\n|$)/g;
...
export function formatDeleteHeader(start: number, end = start): string {
  if (start === end) {
    return `${HL_DELETE_KEYWORD} ${start}`;
  }
  return `${HL_DELETE_KEYWORD} ${start}${HL_RANGE_SEP}${end}`;
}
...
export const HL_LINE_BODY_SEP_RE_RAW = HL_LINE_BODY_SEP.replace(
  REGEX_SPECIAL_CHARS_RE,
  "\\$&"
);
/**
 ...
 *
 * Trailing `[ \t\r]` is trimmed from every line (and the final line) in one
 * pass, so CRLF endings and display-trimmed lines do not invalidate a tag.
 */
export function computeFileHash(text: string): string {
  return createHash("sha256")
    .update(text.replace(TRAILING_INLINE_SPACE_RE, ""))
    .digest("hex")
    .slice(0, HL_FILE_HASH_LENGTH)
    .toUpperCase();
}
...
export function describeAnchorExamples(linePrefix = ""): string {
  if (linePrefix === "") {
    return ["160", "42", "7"].map((example) => `"${example}"`).join(", ");
  }
  const second = linePrefix.slice(0, -1);
  const examples = [linePrefix, `${second === "" ? "4" : second}2`, "7"];
