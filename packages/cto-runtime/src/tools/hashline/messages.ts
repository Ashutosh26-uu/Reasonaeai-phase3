...
// packages/code/src/tools/hashline/messages.ts.
...
export function formatAnchoredContext(
  ...
  for (const line of anchorLines) {
    ...
    const low = Math.max(1, line - MISMATCH_CONTEXT);
    const high = Math.min(fileLines.length, line + MISMATCH_CONTEXT);
    for (let lineNum = low; lineNum <= high; lineNum += 1) {
 ...
 *
 * Ported defect note: in spectra this text is a bare template-literal
 * expression statement immediately after {@link REPLACE_PAIR_COALESCED_WARNING},
 * so it was never bound to a name and could not be used. It is named here
 * (`OVERLAPPED_BARE_HUNK_DROPPED_WARNING`) so the intended constant exists.
 */
export const OVERLAPPED_BARE_HUNK_DROPPED_WARNING = `Dropped a bare hunk overlapped by the concrete hunk after it. One \`SWAP N${HL_RANGE_SEP}M:\` hunk per range — the body is the final content, never old+new.`;
...
export function afterInsertLandingShiftWarning(
  ...
  const plural = crossed === 1 ? "" : "s";
  return `INS.POST ${anchorLine}: body indented shallower than the anchor, so the landing moved past ${crossed} closing line${plural} to after line ${landingLine}. For the deeper position inside the block, re-issue with the body indented to match.`;
}
...
function formatLineRanges(lines: readonly number[]): string {
  ...
  const parts: string[] = [];
  // `sorted` is non-empty (early return above); `?? 0` only satisfies
  // `noUncheckedIndexedAccess`.
  let start = sorted[0] ?? 0;
  let previous = sorted[0] ?? 0;
  for (let index = 1; index <= sorted.length; index += 1) {
    // One past the end on the final iteration; the assignments below are dead
    // then, and the `?? 0` only satisfies `noUncheckedIndexedAccess`.
...
export function unseenLinesMessage(
  ...
  const preview = reveal.lines
    .map(({ line, text }) => `  ${formatNumberedLine(line, text)}`)
    .join("\n");
  if (reveal.truncated) {
...
export function blockSingleLineMessage(line: number, op: BlockOp): string {
  let blockForm = "SWAP.BLK";
  if (op === "insert_after") {
    blockForm = "INS.BLK.POST";
  } else if (op === "delete") {
    blockForm = "DEL.BLK";
  }

  let plainForm = `SWAP ${line}${HL_RANGE_SEP}${line}:`;
  if (op === "insert_after") {
    plainForm = `INS.POST ${line}:`;
  } else if (op === "delete") {
    plainForm = `DEL ${line}`;
  }
