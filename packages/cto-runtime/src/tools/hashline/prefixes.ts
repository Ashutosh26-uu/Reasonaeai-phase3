...
// packages/code/src/tools/hashline/prefixes.ts.
...
function collectLinePrefixStats(lines: string[]): LinePrefixStats {
  ...
  for (const line of lines) {
    ...
    if (HL_PREFIX_RE.test(line)) {
      stats.hashPrefixCount += 1;
    }
    if (HL_PREFIX_PLUS_RE.test(line)) {
      stats.diffPlusHashPrefixCount += 1;
    }
    if (DIFF_PLUS_RE.test(line)) {
      stats.diffPlusCount += 1;
    }
  }
  return stats;
}
...
export function hashlineParseText(
  ...
  if (typeof edit === "string") {
    ...
    return stripNewLinePrefixes(trimmed.replaceAll("\r", "").split("\n"));
  }
