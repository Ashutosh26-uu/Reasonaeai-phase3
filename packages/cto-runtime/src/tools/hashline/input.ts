...
// packages/code/src/tools/hashline/input.ts.
...
/** Trailing `#TAG` snapshot tag at the end of a recovered header body. */
const TRAILING_FILE_HASH_RE = new RegExp(
  `#([0-9A-Fa-f]{${HL_FILE_HASH_LENGTH}})\\s*$`
);

/** Trailing whitespace on a recovered path body. */
...
function tryParseRecoveryHeader(line: string, cwd?: string): RawSection | null {
  ...
  const trailing: RegExpExecArray | null = TRAILING_FILE_HASH_RE.exec(body);
  let pathText: string;
  let fileHash: string | undefined;
  if (trailing === null) {
    pathText = body.replace(TRAILING_WHITESPACE_RE, "");
  } else {
...
function parseHashlineHeaderLine(
  ...
  return token.fileHash === undefined
    ? { diff: "", path: parsedPath }
    : { diff: "", fileHash: token.fileHash, path: parsedPath };
}

/** Trailing carriage return left behind by a CRLF line split. */
const TRAILING_CR_RE = /\r$/;

function stripLeadingBlankLines(input: string): string {
  const stripped = input.startsWith("\uFEFF") ? input.slice(1) : input;
  const lines = stripped.split(LINE_BREAK_RE);
  while (lines.length > 0) {
    const head = (lines[0] ?? "").replace(TRAILING_CR_RE, "");
    if (
      head.trim().length === 0 ||
...
export function containsRecognizableHashlineOperations(input: string): boolean {
  for (const line of input.split(LINE_BREAK_RE)) {
...
/** Unified-diff hunk header, rejected on the first line of a section. */
...
function splitRawSections(
  ...
  if (parseHashlineHeaderLine(firstLine, options.cwd) === null) {
    ...
    if (UNIFIED_HUNK_HEADER_RE.test(firstTrimmed)) {
      throw new Error(
        `unified-diff hunk header (\`@@ -N,M +N,M @@\`) is not valid in hashline. File sections start with \`${HL_FILE_PREFIX}path${HL_FILE_HASH_SEP}HASH${HL_FILE_SUFFIX}\`; use \`replace\`, \`delete\`, or \`insert\` ops.`
      );
    }
    const preview = JSON.stringify(firstLine.slice(0, 120));
    throw new Error(
      `input must begin with "${HL_FILE_PREFIX}PATH${HL_FILE_HASH_SEP}HASH${HL_FILE_SUFFIX}" on the first non-blank line for anchored edits; got: ${preview}. Example: "${HL_FILE_PREFIX}src/foo.ts${HL_FILE_HASH_SEP}${HL_FILE_HASH_EXAMPLES[0]}${HL_FILE_SUFFIX}" then edit ops.`
    );
  }
...
export class PatchSection {
  ...
  parse(): { edits: Edit[]; fileOp?: FileOp; warnings: readonly string[] } {
    ...
    if (parsed.fileOp !== undefined) {
      fileOp =
        parsed.fileOp.kind === "move"
          ? { dest: normalizeHashlinePath(parsed.fileOp.dest), kind: "move" }
          : parsed.fileOp;
    }
    if (fileOp === parsed.fileOp) {
      return parsed;
    }
    return {
      edits: parsed.edits,
  ...
  applyTo(text: string, blockResolver?: BlockResolver): ApplyResult {
    ...
    return merged.length > 0
      ...
      : { firstChangedLine: result.firstChangedLine, text: result.text };
  }
